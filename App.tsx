...
const App: React.FC = () => {
  const [view, setView] = useState<'home' | 'login' | 'shop' | 'checkout' | 'payment-success' | 'terms-of-service' | 'privacy-policy' | 'about-us'>('home');
  ...
  // Check if payment is verified when component mounts
  useEffect(() => {
    const verifiedPaymentStr = localStorage.getItem('verifiedPayment');
    if (verifiedPaymentStr) {
      try {
        const verifiedPayment = JSON.parse(verifiedPaymentStr);
        if (verifiedPayment.verified && verifiedPayment.orderId) {
          setPaymentVerified(true);
        }
      } catch (e) {
        console.error('Failed to parse verified payment:', e);
      }
    }
  }, []);

  // Show success page when user returns from Razorpay (URL has ?payment=success or #payment-success)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const hash = window.location.hash?.replace('#', '') || '';
    if (params.get('payment') === 'success' || hash === 'payment-success') {
      setCart([]);
      setView('payment-success');
      // Clean URL so refresh doesn't re-show success
  ...
  if (view === 'payment-success') {
    // Only show success page if payment is verified
    if (!paymentVerified) {
      setView('home');
      return (
        <div className="antialiased text-earth-900 bg-earth-50 min-h-screen flex flex-col items-center justify-center">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-center"
          >
            <h1 className="text-3xl font-serif text-earth-900 mb-4">Unauthorized Access</h1>
            <p className="text-earth-600 mb-8">Please complete a valid payment to access this page.</p>
            <button
              onClick={() => setView('home')}
              className="px-8 py-3 bg-moss-700 text-white rounded-xl font-semibold hover:bg-moss-800 transition-colors"
            >
              Go Back Home
            </button>
          </motion.div>
      ...
      <main className="flex-grow">
        ...
        <section className="py-32 bg-moss-900 text-moss-50 text-center px-6 relative overflow-hidden">
           ...
           <ScrollReveal className="mx-auto max-w-4xl">
             <h2 className="text-4xl md:text-6xl font-serif mb-8">Know About Us?</h2>
             <p className="text-moss-200 max-w-2xl mx-auto mb-10 text-lg leading-relaxed">
               Learn about Valmiki Manoj Kumar's 7+ years of experience in premium indoor mushroom farming. Discover our journey, expertise, and commitment to quality.
             </p>
             <button 
